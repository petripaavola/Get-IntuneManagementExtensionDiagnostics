	...
	While ($i -lt $LogEntryList.Count) {
			...
			if($LogEntryList[$i].Message -Match '^\[PowerShell\] User Id = (.*), Policy id = (.*), policy result = (.*)$') {
				...
				While (($b -ge 0) -and ($b -gt $PowershellScriptStartLogEntryIndex) -and ($PowershellScriptStartLogEntryFound) -and ($PowershellScriptPolicyIdStart -eq $PowershellScriptPolicyIdEndProcess)) {
					...
					if(($LogEntryList[$b].Message -Match '^write output done. (output =.*)$') -and ($LogEntryList[$b].Component -eq 'AgentExecutor')) {
						...
						While(($LogEntryList[$c].Multiline -ne '---->') -and ($LogEntryList[$c].Component -eq 'AgentExecutor') -and ($c -lt $i)) {
				...
				if($PowershellScriptPolicyResultEndProcess -eq 'Success') {
					...
					if(($PowershellScriptRunTimeTotalSeconds -as [double]) -ne $null -and $PowershellScriptRunTimeTotalSeconds -gt $LongRunningPowershellNotifyThreshold) {
		...
		if($LogEntryList[$i].Message -like 'Get policies = *') {
			if($LogEntryList[$i].Message -Match '^Get policies = \[(.*)\]$') {
				...
				# Save policy snapshot for Targeted Applications table
				$TargetedAppPolicySnapshots.Add([PSCustomObject]@{
					'DateTime' = $LogEntryList[$i].DateTime
					'Policies' = $IntuneAppPolicies
					'Line'     = $LogEntryList[$i].Line
				})
...
#>			
	...
	<style>
	  ...
	  /* Set Targeted Applications table settings */
	  #TargetedAppsTable {
		width: auto;
		max-width: 1200px;
		float: left;
	  ...
	  #TargetedAppsTable tbody tr {
		cursor: pointer;
	  }

	  #TargetedAppsTable tbody tr:hover {
...
"@
		...
		##############################
		# Targeted Applications Table
		##############################
		$TargetedAppsHTML = ''
		if($TargetedAppPolicySnapshots.Count -gt 0) {
			Write-Host "Creating Targeted Applications table..."
			
			# Build list of unique snapshots (only keep ones where app list changed)
			$UniqueSnapshots = [System.Collections.Generic.List[PSObject]]@()
			$previousAppIds = $null
			
			for($s = 0; $s -lt $TargetedAppPolicySnapshots.Count; $s++) {
				$snapshot = $TargetedAppPolicySnapshots[$s]
				$currentAppIds = ($snapshot.Policies | Sort-Object -Property Id | ForEach-Object { "$($_.Id)_v$($_.Version)_i$($_.Intent)" }) -join ','
				
				if($currentAppIds -ne $previousAppIds) {
					$UniqueSnapshots.Add($snapshot)
					$previousAppIds = $currentAppIds
				}
			}
			
			# Determine valid-until times for each unique snapshot
			for($s = 0; $s -lt $UniqueSnapshots.Count; $s++) {
				if($s -lt $UniqueSnapshots.Count - 1) {
					$UniqueSnapshots[$s] | Add-Member -MemberType NoteProperty -Name 'ValidUntil' -Value $UniqueSnapshots[$s + 1].DateTime -Force
				} else {
					$UniqueSnapshots[$s] | Add-Member -MemberType NoteProperty -Name 'ValidUntil' -Value 'Current' -Force
				}
			}
			
			# Build dropdown options and table data as JSON for JavaScript
			$snapshotDataArray = @()
			
			for($s = 0; $s -lt $UniqueSnapshots.Count; $s++) {
				$snap = $UniqueSnapshots[$s]
				$appRows = @()
				
				# Build a lookup by AppId for re-ordering
				$appRowById = @{}
				
				foreach($app in $snap.Policies) {
					# Determine App Type
					if($app.InstallerData) { $appType = 'WinGetApp' } else { $appType = 'Win32App' }
					
					# Intent
					$intentText = switch($app.Intent) {
						0 { 'Not Targeted' }
						1 { 'Available Install' }
						3 { 'Required Install' }
						4 { 'Required Uninstall' }
						default { "Unknown ($($app.Intent))" }
					}
					
					# Detect assignment filter exclusion
					# AppApplicabilityStateDueToAssginmentFilters = 1010 means filter excluded this app
					$isFilteredOut = $false
					$filterNames = ''
					if($app.AssignmentFilterIds -and $app.AssignmentFilterIds.Count -gt 0) {
						if($app.AppApplicabilityStateDueToAssginmentFilters -eq 1010) {
							$isFilteredOut = $true
							$intentText = 'Filter: Not Applicable'
							$filterIds = @()
							foreach($fid in $app.AssignmentFilterIds) {
								if($IdHashtable.ContainsKey($fid)) { $filterIds += $IdHashtable[$fid].displayName }
								else { $filterIds += $fid }
							}
							$filterNames = $filterIds -join ', '
						}
					}
					
					# Target Type
					$targetTypeText = switch($app.TargetType) {
						0 { 'None' }
						1 { 'User' }
						2 { 'Device' }
						default { "Unknown ($($app.TargetType))" }
					}
					
					# ESP Configuration
					$espInfo = ''
					if($app.ESPConfiguration) {
						if($app.ESPConfiguration.IsSelectedApp) { $espInfo = 'ESP Required' }
						elseif($app.ESPConfiguration.InstallWithBestEffort) { $espInfo = 'ESP Best Effort' }
						else { $espInfo = 'Not in ESP' }
					}
					
					# Install Context
					$installContextText = switch($app.InstallContext) {
						1 { 'System' }
						2 { 'User' }
						default { "Unknown ($($app.InstallContext))" }
					}
					
					# Build detail tooltip (full app details)
					$detailLines = @()
					$detailLines += "Application Details"
					$detailLines += "==================="
					$detailLines += "Name: $($app.Name)"
					$detailLines += "Id: $($app.Id)"
					$detailLines += "Version: $($app.Version)"
					$detailLines += "App Type: $appType"
					$detailLines += "Intent: $intentText"
					if($isFilteredOut -and $filterNames) {
						$detailLines += "Assignment Filter (excluded): $filterNames"
					} elseif($app.AssignmentFilterIds -and $app.AssignmentFilterIds.Count -gt 0 -and -not $isFilteredOut) {
						$fids = @()
						foreach($fid in $app.AssignmentFilterIds) {
							if($IdHashtable.ContainsKey($fid)) { $fids += $IdHashtable[$fid].displayName } else { $fids += $fid }
						}
						$detailLines += "Assignment Filter (included): $($fids -join ', ')"
					}
					$detailLines += "Target Type: $targetTypeText"
					$detailLines += "Install Context: $installContextText"
					$detailLines += "ESP: $espInfo"
					$detailLines += ""
					
					# Install/Uninstall commands
					if($app.InstallCommandLine) {
						$detailLines += "Install Command: $($app.InstallCommandLine)"
					}
					if($app.UninstallCommandLine) {
						$detailLines += "Uninstall Command: $($app.UninstallCommandLine)"
					}
					if($app.SetUpFilePath) {
						$detailLines += "Setup File: $($app.SetUpFilePath)"
					}
					$detailLines += ""
					...
					if($app.InstallEx) {
						try {
							$installEx = $app.InstallEx | ConvertFrom-Json -ErrorAction SilentlyContinue
							if($installEx) {
								$runAsText = switch($installEx.RunAs) { 1 { 'System' } 2 { 'User' } default { $installEx.RunAs } }
								$restartText = switch($installEx.DeviceRestartBehavior) { 0 { 'Determine by return codes' } 1 { 'No specific action' } 2 { 'App install may force restart' } 3 { 'Force restart' } default { $installEx.DeviceRestartBehavior } }
								$detailLines += "Install Settings:"
								$detailLines += "  Run As: $runAsText"
								$detailLines += "  Max Run Time: $($installEx.MaxRunTimeInMinutes) min"
								$detailLines += "  Max Retries: $($installEx.MaxRetries)"
								$detailLines += "  Restart Behavior: $restartText"
								$detailLines += ""
							}
						} catch {}
					}
					...
					if($app.RequirementRules) {
						try {
							$reqRules = $app.RequirementRules | ConvertFrom-Json -ErrorAction SilentlyContinue
							if($reqRules) {
								$detailLines += "Requirement Rules:"
								$osArchText = switch($reqRules.RequiredOSArchitecture) { 1 { 'x86' } 2 { 'x64' } 3 { 'x86/x64' } default { $reqRules.RequiredOSArchitecture } }
								$detailLines += "  OS Architecture: $osArchText"
								if($reqRules.MinimumWindows10BuildNumer) { $detailLines += "  Min OS Version: $($reqRules.MinimumWindows10BuildNumer)" }
								if($reqRules.MinimumFreeDiskSpaceInMB) { $detailLines += "  Min Free Disk: $($reqRules.MinimumFreeDiskSpaceInMB) MB" }
								if($reqRules.MinimumMemoryInMB) { $detailLines += "  Min Memory: $($reqRules.MinimumMemoryInMB) MB" }
								$detailLines += ""
							}
						} catch {}
					}
					
					# Extended Requirement Rules (may contain PowerShell scripts)
					if($app.ExtendedRequirementRules -and $app.ExtendedRequirementRules -ne '[]') {
						try {
							$extReqRules = $app.ExtendedRequirementRules | ConvertFrom-Json -ErrorAction SilentlyContinue
							if($extReqRules -and $extReqRules.Count -gt 0) {
								$detailLines += "Extended Requirement Rules:"
								foreach($extRule in $extReqRules) {
									if($extRule.ScriptBody) {
										$detailLines += "  PowerShell Script Requirement:"
										try {
											$decodedScript = [System.Text.Encoding]::UTF8.GetString([System.Convert]::FromBase64String($extRule.ScriptBody))
											$detailLines += "  --- Script Content ---"
											$detailLines += $decodedScript
											$detailLines += "  --- End Script ---"
										} catch {
											$detailLines += "  Script (Base64): $($extRule.ScriptBody)"
										}
									} else {
										$detailLines += "  Rule: $($extRule | ConvertTo-Json -Compress -Depth 2)"
									}
								}
								$detailLines += ""
							}
					...
					# Detection Rules
					if($app.DetectionRule) {
						try {
							$detRules = $app.DetectionRule -replace '\\\\', '\\' | ConvertFrom-Json -ErrorAction SilentlyContinue
							if($detRules) {
								$detailLines += "Detection Rules:"
								foreach($detRule in $detRules) {
									$detTypeText = switch($detRule.DetectionType) {
										0 { 'Registry' }
										1 { 'MSI' }
										2 { 'File' }
										3 { 'Custom Script' }
										default { "Type $($detRule.DetectionType)" }
									}
									$detailLines += "  Detection Type: $detTypeText"
									
									if($detRule.DetectionText) {
										$detText = $detRule.DetectionText -replace '\\\\', '\\' | ConvertFrom-Json -ErrorAction SilentlyContinue
										if($detText) {
											if($detRule.DetectionType -eq 2) {
												# File detection
												$detailLines += "    Path: $($detText.Path)"
												$detailLines += "    File: $($detText.FileOrFolderName)"
												if($detText.DetectionValue) { $detailLines += "    Version: $($detText.DetectionValue)" }
											} elseif($detRule.DetectionType -eq 0) {
												# Registry detection
												if($detText.KeyPath) { $detailLines += "    Key: $($detText.KeyPath)" }
												if($detText.ValueName) { $detailLines += "    Value: $($detText.ValueName)" }
												if($detText.DetectionValue) { $detailLines += "    Data: $($detText.DetectionValue)" }
											} elseif($detRule.DetectionType -eq 3) {
												# Script detection
												if($detText.ScriptBody) {
													try {
														$decodedDetScript = [System.Text.Encoding]::UTF8.GetString([System.Convert]::FromBase64String($detText.ScriptBody))
														$detailLines += "    --- Detection Script ---"
														$detailLines += $decodedDetScript
														$detailLines += "    --- End Detection Script ---"
													} catch {
														$detailLines += "    Script (Base64): $($detText.ScriptBody)"
													}
												}
											} elseif($detRule.DetectionType -eq 1) {
												# MSI detection
												if($detText.ProductCode) { $detailLines += "    Product Code: $($detText.ProductCode)" }
												if($detText.ProductVersion) { $detailLines += "    Version: $($detText.ProductVersion)" }
											}
										}
									}
								}
					...
					if($app.ReturnCodes) {
						try {
							$returnCodes = $app.ReturnCodes | ConvertFrom-Json -ErrorAction SilentlyContinue
							if($returnCodes) {
								$detailLines += "Return Codes:"
								foreach($rc in $returnCodes) {
									$rcTypeText = switch($rc.Type) { 0 { 'Failed' } 1 { 'Success' } 2 { 'Soft Reboot' } 3 { 'Hard Reboot' } 4 { 'Retry' } default { "Type $($rc.Type)" } }
									$detailLines += "  $($rc.ReturnCode) = $rcTypeText"
								}
								$detailLines += ""
							}
						} catch {}
					...
					# Dependencies
					if($app.FlatDependencies) {
						$detailLines += "Dependencies:"
						foreach($dep in $app.FlatDependencies) {
							$depActionText = switch($dep.Action) { 10 { 'Install' } 110 { 'Auto Install' } default { "Action $($dep.Action)" } }
							$depTypeText = switch($dep.Type) { 0 { 'Dependency' } 1 { 'Supersedence' } default { "Type $($dep.Type)" } }
							$depName = if($IdHashtable.ContainsKey($dep.ChildId)) { $IdHashtable[$dep.ChildId].Name } else { $dep.ChildId }
							$detailLines += "  $depTypeText ($depActionText): $depName"
						}
						$detailLines += ""
					}
					
					# Escape detail for HTML
					$detailText = ($detailLines -join "`n")
					# HTML-encode special characters for safe embedding
					$detailTextEscaped = $detailText -replace '&','&amp;' -replace '<','&lt;' -replace '>','&gt;' -replace '"','&quot;' -replace "'","&#39;"
					
					$appRow = [PSCustomObject]@{
						'Name'         = $app.Name
						'Intent'       = $intentText
						'TargetType'   = $targetTypeText
						'AppType'      = $appType
						'InstallCtx'   = $installContextText
						'ESP'          = $espInfo
						'AppId'        = $app.Id
						'Detail'       = $detailTextEscaped
						'IsNotTargeted'= ($app.Intent -eq 0)
						'IsFilteredOut'= $isFilteredOut
						'FilterNames'  = $filterNames
						'SupersededIds'= @($app.FlatDependencies | Where-Object { $_.Type -eq 1 } | ForEach-Object { $_.ChildId })
					}
					$appRows += $appRow
					$appRowById[$app.Id] = $appRow
				}
				
				# Re-order: main/targeted apps sorted by name, each followed by their superseded children.
				# Apps that are superseded children of another app are grouped under their parent.
				# Remaining Not Targeted apps not claimed by any parent go at the end.
				$supersededChildIds = [System.Collections.Generic.HashSet[string]]@()
				foreach($row in $appRows) {
					foreach($sid in $row.SupersededIds) { [void]$supersededChildIds.Add($sid) }
				}
				
				# Helper: recursively append a node and all its superseded descendants (depth-first)
				$addedIds = [System.Collections.Generic.HashSet[string]]@()
				function Add-AppRowRecursive($row, [ref]$orderedList) {
					if($addedIds.Contains($row.AppId)) { return }
					[void]$addedIds.Add($row.AppId)
					$orderedList.Value += $row
					# Add superseded children sorted by name, then recurse into each
					$children = $row.SupersededIds | ForEach-Object {
						if($appRowById.ContainsKey($_)) { $appRowById[$_] }
					} | Where-Object { $_ -ne $null } | Sort-Object -Property Name
					foreach($child in $children) {
						Add-AppRowRecursive $child $orderedList
					}
				}
				
				$orderedAppRows = @()
				# First pass: all non-superseded-child apps sorted by name, each followed by full descendant tree
				$mainApps = $appRows | Where-Object { -not $supersededChildIds.Contains($_.AppId) } | Sort-Object -Property Name
				foreach($mainApp in $mainApps) {
					Add-AppRowRecursive $mainApp ([ref]$orderedAppRows)
				}
				# Second pass: any orphaned superseded apps not yet added - append at end
				foreach($row in ($appRows | Where-Object { $supersededChildIds.Contains($_.AppId) -and -not $addedIds.Contains($_.AppId) } | Sort-Object -Property Name)) {
					Add-AppRowRecursive $row ([ref]$orderedAppRows)
				}
				
				$targetedAppCount = ($snap.Policies | Where-Object { $_.Intent -ne 0 }).Count
				$snapshotDataArray += [PSCustomObject]@{
					'Index'      = $s
					'Label'      = "Policy $($s + 1): $($snap.DateTime) ($targetedAppCount apps) - Valid until: $($snap.ValidUntil)"
					'Apps'       = $orderedAppRows
				}
			}
			
			# Build the HTML
			$TargetedAppsPreContent = @"
		<div style="clear:both;"></div>
		<h2 id="TargetedApplications">Targeted Applications (Intune App Policies)</h2>
		<p style="color: #333; margin-bottom: 10px;">Select a policy snapshot to view targeted applications. Only snapshots with changes are shown.</p>
		<div style="margin-bottom: 10px;">
			<label for="policySnapshotSelect" style="font-weight: bold; color: #333;">Policy Snapshot: </label>
			<select id="policySnapshotSelect" onchange="filterTargetedAppsTable()" style="min-width: 400px; padding: 4px;">
"@
			
			foreach($snapData in $snapshotDataArray) {
				$TargetedAppsPreContent += "`n				<option value=`"$($snapData.Index)`">$($snapData.Label)</option>"
			}
...
"@
			...
			# Build table rows as HTML directly (all snapshots, hidden by data attribute)
			$tableHeaderHTML = @"
		<table id="TargetedAppsTable">
		<thead><tr>
			<th>Name</th>
			<th>Intent</th>
			<th>Target Type</th>
			<th>App Type</th>
			<th>Install Context</th>
			<th>ESP</th>
		</tr></thead>
		<tbody>
"@
			...
			$tableBodyHTML = ''
			foreach($snapData in $snapshotDataArray) {
				foreach($appRow in $snapData.Apps) {
					# Color coding based on intent
					$rowColor = switch($appRow.Intent) {
						'Required Install'      { '#0d3a6e' }
						'Available Install'     { '#1a4a5e' }
						'Required Uninstall'    { '#5e1a2a' }
						'Not Targeted'          { '#1a2040' }
						'Filter: Not Applicable'{ '#3a2a00' }
						default                 { '' }
					}
					$rowStyle = if($rowColor) { " style=`"background-color: $rowColor;`"" } else { '' }
					$displayStyle = if($snapData.Index -eq 0) { '' } else { ' style="display:none;"' }
					# Combine styles
					if($snapData.Index -ne 0 -and $rowColor) {
						$combinedStyle = " style=`"display:none; background-color: $rowColor;`""
					} elseif($snapData.Index -ne 0) {
						$combinedStyle = ' style="display:none;"'
					} elseif($rowColor) {
						$combinedStyle = " style=`"background-color: $rowColor;`""
					} else {
						$combinedStyle = ''
					}
					
					$tableBodyHTML += "		<tr data-snapshot=`"$($snapData.Index)`" data-detail=`"$($appRow.Detail)`"$combinedStyle onclick=`"showAppDetail(this)`">"
					if($appRow.IsFilteredOut) {
						# Filtered out by assignment filter - icon but no indent
						$filterTip = if($appRow.FilterNames) { " title=`"Filter: $($appRow.FilterNames)`"" } else { '' }
						$tableBodyHTML += "<td><span style=`"opacity:0.6; margin-right:4px;`"$filterTip>&#x26D4;</span>$($appRow.Name)</td>"
					} elseif($appRow.IsNotTargeted) {
						# Not targeted / superseded - indent with arrow
						$tableBodyHTML += "<td style=`"padding-left: 24px;`"><span style=`"opacity:0.5; margin-right:4px;`">&#x21B3;</span>$($appRow.Name)</td>"
					} else {
						$tableBodyHTML += "<td>$($appRow.Name)</td>"
					}
					$tableBodyHTML += "<td>$($appRow.Intent)</td>"
					$tableBodyHTML += "<td>$($appRow.TargetType)</td>"
					$tableBodyHTML += "<td>$($appRow.AppType)</td>"
					$tableBodyHTML += "<td>$($appRow.InstallCtx)</td>"
					$tableBodyHTML += "<td>$($appRow.ESP)</td>"
					$tableBodyHTML += "</tr>`n"
				}
			}
...
"@
			...
			# Modal popup for app details (light theme matching main table tooltips)
			$modalHTML = @"
		<div id="appDetailModal" style="display:none; position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.3); z-index:9999;">
			<div style="position:relative; margin:5% auto; width:80%; max-width:900px; max-height:80vh; background:#f9f9f9; border:1px solid #ccc; border-radius:6px; overflow:auto; box-shadow:0 4px 8px rgba(0,0,0,0.1);">
				<div style="position:sticky; top:0; background:linear-gradient(to bottom, #5592bb 0%, #327cad 66%, #1C6EA4 100%); padding:10px 15px; display:flex; justify-content:space-between; align-items:center;">
					<h3 style="margin:0; color:#fff;">Application Details</h3>
					<span onclick="document.getElementById('appDetailModal').style.display='none'" style="color:#fff; cursor:pointer; font-size:24px; font-weight:bold;">&times;</span>
				</div>
				<pre id="appDetailContent" style="padding:15px; margin:0; white-space:pre-wrap; font-family:Consolas,'Courier New',monospace; font-size:14px; color:#333;"></pre>
			</div>
		</div>
"@
			...
			$targetedAppsJS = @"
		<script>
		function filterTargetedAppsTable() {
			var sel = document.getElementById('policySnapshotSelect');
			var idx = sel.value;
			var table = document.getElementById('TargetedAppsTable');
			var rows = table.getElementsByTagName('tbody')[0].getElementsByTagName('tr');
			for (var i = 0; i < rows.length; i++) {
				if (rows[i].getAttribute('data-snapshot') === idx) {
		...
		function showAppDetail(row) {
			var detail = row.getAttribute('data-detail');
			// Decode HTML entities
			var txt = document.createElement('textarea');
			txt.innerHTML = detail;
			document.getElementById('appDetailContent').textContent = txt.value;
...
"@
			...
			$TargetedAppsHTML = $TargetedAppsPreContent + $tableHeaderHTML + $tableBodyHTML + $tableFooterHTML + $modalHTML + $targetedAppsJS
		}


		$Title = "Get-IntuneManagementExtensionDiagnostics Observed Timeline Report"
		ConvertTo-HTML -head $head -PostContent $observedTimelineHTML, $ApplicationDownloadStatisticsHTML, $TargetedAppsHTML, $JavascriptPostContent -PreContent $PreContent -Title $Title | Out-File "$ReportSavePath\$HTMLFileName"
		$Success = $?
